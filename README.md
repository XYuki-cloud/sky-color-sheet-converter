...
三个启动器都使用项目相对路径，并优先调用项目 `.venv`，项目移动到其他文件夹后仍可使用。Windows 快捷方式 `.lnk` 不属于源码项目，仓库不会保存绑定本机路径的快捷方式。
...
本机运行时还可能出现以下目录，但它们不属于公开源码：`.venv/`、`.audio-venv/`、`outputs/`、`logs/`、模型 checkpoint、音视频和 Codex 临时文件。`.gitignore` 已将这些内容排除。
...
- Tsumugi 以 Git 子模块提供源码引用，固定版本和模型边界见 [`THIRD_PARTY_NOTICES.md`](THIRD_PARTY_NOTICES.md)；
...
本项目源码使用 [Apache License 2.0](LICENSE)。第三方子模块、模型、依赖和用户输入内容不随本许可证重新授权。
