...
COVER_TITLE_COLOR = "#FFD447"
COVER_TITLE_STROKE_COLOR = "#111827"
COVER_TITLE_MAX_SIZE = 112
COVER_TITLE_MIN_SIZE = 22
...
def _cover_title_font(
    draw: ImageDraw.ImageDraw,
    title: str,
    max_width: int,
) -> tuple[ImageFont.ImageFont, int, int]:
    """Choose a readable outlined title font that fits a cover page."""

    upper_size = max(24, min(COVER_TITLE_MAX_SIZE, max_width // 5))
    for size in range(upper_size, COVER_TITLE_MIN_SIZE - 1, -2):
        font = _title_font(size)
        stroke_width = max(2, round(size * 0.06))
        ...
        if bounds[2] - bounds[0] <= max_width:
            return font, stroke_width, bounds[3] - bounds[1]
    font = _title_font(COVER_TITLE_MIN_SIZE)
    stroke_width = max(1, round(COVER_TITLE_MIN_SIZE * 0.06))
    bounds = draw.multiline_textbbox(
        (0, 0),
        title,
        font=font,
        stroke_width=stroke_width,
        spacing=max(2, COVER_TITLE_MIN_SIZE // 8),
        align="center",
    )
    return font, stroke_width, bounds[3] - bounds[1]
...
def render_cover_page(
    ...
    *,
    ...
    """Copy the first rendered page and overlay a centered yellow title.

    Cover pages are presentation/upload artifacts.  They are not added to the
    logical ``images`` array, so a player never treats the cover as a playable
    frame.  The ``000`` filename is assigned by the page-family renderers.
    """

    first_page = Path(first_page_path)
    output = Path(output_path)
    output.parent.mkdir(parents=True, exist_ok=True)
    with Image.open(first_page) as source_image:
        image = source_image.convert("RGB")

    page_title = (title or first_page.stem).strip() or "未命名曲谱"
    draw = ImageDraw.Draw(image)
    font, stroke_width, text_height = _cover_title_font(
        draw,
        page_title,
        max(1, image.width - 40),
    )
    spacing = max(2, getattr(font, "size", COVER_TITLE_MIN_SIZE) // 8)
    bounds = draw.multiline_textbbox(
        (0, 0),
        page_title,
        font=font,
    ...
    text_width = bounds[2] - bounds[0]
    text_height = max(text_height, bounds[3] - bounds[1])
    x = (image.width - text_width) / 2 - bounds[0]
    center_y = round(image.height * 0.55)
    y = center_y - text_height / 2 - bounds[1]
    draw.multiline_text(
        (x, y),
        page_title,
        font=font,
        fill=COVER_TITLE_COLOR,
        stroke_width=stroke_width,
        stroke_fill=COVER_TITLE_STROKE_COLOR,
        spacing=spacing,
        align="center",
    )
...
def _with_cover_page(
    page_paths: Sequence[Path],
    output_path: Path,
    *,
    title: str,
) -> tuple[Path, ...]:
    """Return ``000`` cover followed by the original numbered pages."""

    if not page_paths:
        return ()
    cover_path = render_cover_page(page_paths[0], output_path, title=title)
    return (cover_path, *page_paths)
...
def render_color_pages(
    ...
    *,
    ...
    return _with_cover_page(
        page_paths,
        output / f"{stem}.color-000.png",
        title=page_title,
    )
...
def render_mobile_color_pages(
    ...
    *,
    ...
    return _with_cover_page(
        page_paths,
        output / f"{stem}.color-mobile-000.png",
        title=page_title,
    )
...
def convert_sky_txt_file(
    ...
    *,
    ...
    payload["artifacts"] = {
        ...
        "cover_png": (
            mobile_page_paths[0].name if mobile_page_paths else None
        ),
        "desktop_cover_png": page_paths[0].name if page_paths else None,
    }
    json_path.write_text(
        json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
