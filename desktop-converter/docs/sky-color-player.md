...
默认输出结构化 JSON、手机竖版分页 PNG、音符侧车和报告。每首歌会先生成 `*-000.png` 黄色标题封面，再生成 `*-001.png` 等谱页；封面沿用第一张谱页的网格，只叠加黄色曲名，且不计入谱页右上角的 `当前页/总页数`（例如 `1/10`）。手机竖版每页最多 32 张逻辑图、4 列排列。手机版只绘制标题、页码和无按键文字的 5×3 方格，不渲染字幕。封面只属于 PNG 发布产物，不在 JSON 的 `images` 中，播放器不会播放它。横版分页 PNG 默认不生成；给转换命令添加 `--desktop-pages` 后才会额外生成。
