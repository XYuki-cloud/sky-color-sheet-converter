...
默认输出结构化 JSON、手机竖版分页 PNG、音符侧车和报告。手机竖版每页最多 32 张逻辑图、4 列排列，右上角显示 `当前页/总页数`，例如 `1/10`。手机版只绘制标题、页码和无按键文字的 5×3 方格，不渲染字幕。横版分页 PNG 默认不生成；给转换命令添加 `--desktop-pages` 后才会额外生成。
