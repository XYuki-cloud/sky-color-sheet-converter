...
def test_converter_writes_versioned_json_and_24_image_png_pages(tmp_path: Path):
    ...
    assert mobile_pages == ["sample_song.color-mobile-001.png"]
    assert (output_dir / mobile_pages[0]).is_file()
...
def test_mobile_color_pages_use_four_columns_and_no_cell_labels(tmp_path: Path):
    ...
    assert len(pages) == 2
    assert pages[0].name == "song.color-mobile-001.png"
    assert pages[1].name == "song.color-mobile-002.png"
    first = Image.open(pages[0]).convert("RGB")
    second = Image.open(pages[1]).convert("RGB")
